...
for frame in cam.capture_continuous(rawcapture, format = "bgr", use_video_port=True):
    ...
    if has_door_been_init == False: #Ran only at the beginning
        if door_debug==False:
            ...
            #doors=np.array([[0,50,100,200]])#One door
            doors = []
    ...
    if zero_f_exists == False: #First frame
        ...
        for object in doors: #Object is door num
            ...
            delta_door_occ= logic(ped_len_o,ped_len_n,ovlp_o[ct_d],ovlp_n_v)
            door_occup[ct_d] += np.int16(delta_door_occ) #What if size(ovlp_n)!=size(ovlp_o)
